...
%\begin{itemize}
%\item Parametric models give us a way to summarize the data. Once we know the right parameters, we can describe the data using those parameters instead of the entire dataset.
%\item Maybe we don't want to store the entire dataset.
%\item Many common models and hypothesis tests rely on parametric assumptions about the data. Linear regression models, the T-test, and the chi-squared test are just a few examples.
%\item Making parametric assumptions means we need fewer samples than we would need to fully specify some general distribution.
%\item We don't over-fit to particularities of our own datasets, such as outliers, etc.
%\end{itemize}
\end{enumerate}
