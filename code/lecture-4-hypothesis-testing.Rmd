...
p <- ggplot() + geom_line(aes(x = z, y = dnorm(z, mu, sigma)), colour = "gray30") + 
  xlab("Systolic Blood Pressure (mmHg)") + theme_bw() + ylab("Density") + 
...
p <- ggplot(df_z_mu) + 
  ...
  geom_ribbon(aes(x = z, ymin = 0, ymax = y1), fill = "#dc298d", 
              data = subset(df_z_mu, z > l1))
ggsave(p, filename = "../img/hyp-z-test-example-6.png", height=3, width=6, units="in", dpi=300)
```
...
p <- ggplot(df_z_mu) + 
  ...
  ylab("Density")
ggsave(p, filename = "../img/hyp-z-test-example-6a.png", height=3, width=6, units="in", dpi=300)
```
...
ggsave(p, filename = "../img/hyp-z-test-example-6b.png", height=3, width=6, units="in", dpi=300)
```
...
ggsave(p, filename = "../img/hyp-z-test-example-6c.png", height=3, width=6, units="in", dpi=300)
```
...
dens <- dchisq(x, 2)
p <- ggplot() + geom_density(aes(x=x, y = dens), stat = "identity", 
                             fill = "blue", colour = NA, alpha = 0.6) + theme_bw() + 
...
e <- e[1:50,]
mean_bmi <- mean(e$bmi)
p <- ggplot(e) + geom_histogram(aes(x = bmi), fill = "gray60", bins=50) + theme_bw() + 
  xlab("BMI") + ylab("Count") + 
...
x = seq(-4, 4, 0.01)
y = dnorm(x, 0, 1)
p <- ggplot() + geom_line(aes(x = x, y = y), colour = "gray60") + theme_bw() + 
...
p <- ggplot(dat) + 
  ...
  theme_bw() + xlab(expression("\u03C7"^2)) + ylab("Density") + 
...
p <- ggplot(e) + 
  geom_histogram(aes(x = hemoglobin), fill = "#06abeb", alpha = 0.5, 
                 data = subset(e, CardiovascularDisease == 1), bins=50) + theme_bw() + 
