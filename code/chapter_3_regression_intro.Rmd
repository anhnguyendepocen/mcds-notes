...
png(filename = "../img/esl-decision-tree-just-tree-reg.png", height=4, width=4, units="in", res=300)
rpart.plot(m, box.palette=palette(20))
dev.off()
```

Plots of the individual covariates vs. the outcome.

```{r}
p <- ggplot(df) + 
  geom_point(aes(x = x1, y = y, colour = y)) + 
  theme_bw() + 
  scale_colour_gradient(low = "#00bfc4", high = "#f8766d", guide = FALSE) 
print(p)
ggsave(p, filename = "../img/esl-reg-x1.png", height=4, width=4, units="in", dpi=300)

p <- ggplot(df) + 
  geom_point(aes(x = x2, y = y, colour = y)) + 
  theme_bw() + 
  scale_colour_gradient(low = "#00bfc4", high = "#f8766d", guide = FALSE) 
print(p)
ggsave(p, filename = "../img/esl-reg-x2.png", height=4, width=4, units="in", dpi=300)
```
